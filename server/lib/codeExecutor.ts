...
// Language runtime metadata and file naming conventions
...
const SUPPORTED_LANGUAGES: Record<string, LanguageMetadata> = {
...
function truncateOutput(text: string): { content: string; wasTruncated: boolean } {
  if (!text) return { content: '', wasTruncated: false };
 ...
 */
export async function executeCode(params: CodeExecutionParams): Promise<CodeExecutionResult> {
