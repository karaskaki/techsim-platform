...
router.post('/submissions', auth_1.authMiddleware, (0, planGuard_1.requirePlan)('free'), async (req, res, next) => {
    try {
        if (!req.user) {
        ...
        const problem = await lldSubmission_1.LLDProblem.findById(problemId);
        if (!problem) {
            return res.status(404).json({ error: 'Problem not found' });
        }
        // Upsert or create submission for this user and problem
        let submission = await lldSubmission_1.LLDSubmission.findOne({
        ...
        if (submission) {
            ...
            submission = new lldSubmission_1.LLDSubmission({
                userId: req.user.userId,
                problemId: problem._id,
                language,
                code,
...
router.post('/ai-generate', auth_1.authMiddleware, (0, planGuard_1.checkLimit)('aiGenerationsPerDay'), rateLimiter_1.aiLimiter, async (req, res, next) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        ...
        const generatedProblem = await (0, ai_1.generateLLDProblem)(topic.trim(), difficulty, groqKey, geminiKey);
        if (saveToDb) {
            const problem = await lldSubmission_1.LLDProblem.findOneAndUpdate({ slug: generatedProblem.slug }, generatedProblem, { upsert: true, new: true, setDefaultsOnInsert: true });
...
router.post('/ai-review', auth_1.authMiddleware, (0, planGuard_1.checkLimit)('aiGenerationsPerDay'), rateLimiter_1.aiLimiter, async (req, res, next) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        ...
        if (isStreaming) {
            ...
            let fullReview = '';
            try {
                fullReview = await (0, ai_1.reviewLLDSubmission)(code, language || 'python', { title: problemTitle, description: problemDescription }, (chunk) => {
                    res.write(`data: ${JSON.stringify({ chunk })}\n\n`);
                }, groqKey, geminiKey);
                if (submissionId) {
                    await lldSubmission_1.LLDSubmission.findByIdAndUpdate(submissionId, { aiReview: fullReview }).catch(() => { });
                }
                res.write('data: [DONE]\n\n');
                res.end();
            }
        ...
        const review = await (0, ai_1.reviewLLDSubmission)(code, language || 'python', { title: problemTitle, description: problemDescription }, undefined, groqKey, geminiKey);
        // If a submissionId was provided, attach the AI review to the submission
        if (submissionId) {
            await lldSubmission_1.LLDSubmission.findByIdAndUpdate(submissionId, { aiReview: review });
        }
...
router.get('/progress', auth_1.authMiddleware, async (req, res, next) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        let progress = await lldSubmission_1.LLDProgress.findOne({ userId: req.user.userId }).populate('completedProblems', 'title slug difficulty patternTags');
        if (!progress) {
            progress = await lldSubmission_1.LLDProgress.create({
                userId: req.user.userId,
                completedProblems: [],
...
router.post('/progress', auth_1.authMiddleware, async (req, res, next) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        const { completedProblemId, currentRoadmapStep, solidPrincipleQuizScore } = req.body;
        let progress = await lldSubmission_1.LLDProgress.findOne({ userId: req.user.userId });
        if (!progress) {
            progress = new lldSubmission_1.LLDProgress({
                userId: req.user.userId,
                completedProblems: [],
                currentRoadmapStep: 0,
        ...
        if (completedProblemId) {
            const alreadyCompleted = progress.completedProblems.some((id) => id.toString() === completedProblemId.toString());
            if (!alreadyCompleted) {
        ...
        if (solidPrincipleQuizScore && typeof solidPrincipleQuizScore.principle === 'string' && typeof solidPrincipleQuizScore.score === 'number') {
            if (!progress.solidPrincipleQuizScores) {
                progress.solidPrincipleQuizScores = {};
            }
            progress.solidPrincipleQuizScores.set(solidPrincipleQuizScore.principle, solidPrincipleQuizScore.score);
        }
        await progress.save();
